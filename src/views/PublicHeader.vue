<template>
  <div class="th">
    <nav class="Nav">
      <router-link class="Header_left" to="/">
        <i class="Header_Logo"></i>
        <div class="Logo_title">绘星工具箱-"帮小忙,也能帮大忙"</div>
      </router-link>

      <div>
        <router-link to="/"><el-tag class="b">全部工具</el-tag></router-link> |
        <router-link to="/clouddisk"
          ><el-tag class="b1" type="success">云盘</el-tag></router-link
        >|
        <router-link to="/candv"
          ><el-tag class="b2" type="success">CV字符串</el-tag></router-link
        >
        |

        <router-link to="/TodoListContent"
          ><el-tag class="b2" type="success">ToDoList</el-tag></router-link
        >
        |
        <router-link to="/RandomText"
          ><el-tag class="b1" type="success">随机文案</el-tag></router-link
        >
        |
        <router-link to="/pwdval"
          ><el-tag class="b2" type="success">密码格式验证</el-tag></router-link
        >
        |
      </div>
    </nav>
    <div
      class="header_txtA"
      v-show="
        $route.path == '/' 
      "
    >
      <div class="title_A">"{{ passageToltal }}"</div>

      <ShareLink />
    </div>


    <SearchView />

    <el-backtop></el-backtop>
    <keep-alive>
      <router-view />
    </keep-alive>
  </div>
</template>

<script>
import SearchView from "./SearchView.vue";

export default {
  name: "pubheader",
  // mounted(){
  //     // setInterval(this.passageToltal,3000);
  // }, 
  data() {
    
    return {
      passage: [
        "人生就像骑自行车，要保持平衡，就必须停止踩踏板。",
        "没有口水与汗水，就没有成功的泪水。",
        "最困难的事情就是认识自己，最容易的事情就是忘记自己。",
        "只有你学会把自己已有的成绩都归零，才能腾出空间去接纳更多的新东西，如此才能使自己不断的超越自己。",
        "莫向不幸屈服，应该更大胆、更积极地向不幸挑战。",
        "人生并非都是选择题，而是在做应用题，那要我们一点一滴的去论证，是取舍的过程，做错了也没关系。向前走，不必想太多，也不要害怕，相信梦想并坚持。",
        "过去的事已经一去不复返。聪明的人是考虑现在和未来，根本无暇去想过去的事。",
        "环境不会改变，解决之道在于改变自己。",
        "忍别人所不能忍的痛，吃别人所别人所不能吃的苦，是为了收获得不到的收获。",
        "如果不想做点事情，就甭想到达这个世界上的任何地方。",
        "行动是成功的阶梯，行动越多，登得越高。",
        "生命之灯因热情而点燃，生命之舟因拼搏而前行。",
        "不要问别人为你做了什么，而要问你为别人做了什么。",
        "在泪水中浸泡过的微笑最灿烂，从迷惘中走出来的灵魂最清醒。",
        "当一个小小的心念变成成为行为时，便能成了习惯；从而形成性格，而性格就决定你一生的成败。这个世界并不是掌握在那些嘲笑者的手中，而恰恰掌握在能够经受得住嘲笑与批评忍不断往前走的人手中。",
        "成功的信念在人脑中的作用就如闹钟，会在你需要时将你唤醒。",
        "最重要的就是不要去看远方模糊的，而要做手边清楚的事。",
        "昨天下了雨，今天刮了风，明天太阳就会出来了。",
        "行动不一定带来快乐，而无行动则决无快乐。",
        "虽然行动不一定能带来满意的结果，但是不行动就绝对不会有满意的结果。",
        "逆境中，力挽狂澜使强者更强，随波逐流使弱者更弱。",
        "多数人都拥有自己不了解的能力和机会，都有可能做到未曾梦想的事情。",
        "每一个成功者都有一个开始。勇于开始，才能找到成功的路。",
        "世界会向那些有目标和远见的人让步。",
        "追风赶月莫停留，平芜尽处是春山。",
        "过去属于死神，未来属于你自己。",
        "只有不断找寻机会的人才会及时把握机会。",
        "高峰只对攀登它而不是仰望它的人来说才有真正意义。",
        "只要站起来比倒下去多一次就是成功。",
        "腰板挺得笔直的人，终究不会走在攀登者队伍的前列。",
        "虽然我们无法改变人生，但可以改变人生观；虽然我们无法改变环境，但我们可以改变心境！",
        "生活不相信眼泪，眼泪并不代表软弱。",
        "珍惜今天，把握现在，因为明天和意外，不知哪个先来。",
        "人生就像一盒巧克力，你永远不知道你会吃到什么口味，每天都要吃点苦的东西，以免忘记苦味。",
        "成功的秘诀端赖坚毅的决心。",
      ],
      visible: false,
    };
  },
  components: {
    SearchView,
  },

 
  computed:{
    passageToltal(){
      var rdom =  Math.floor(Math.random() * this.passage.length-1)
      var printyuju = this.passage[parseInt(rdom)];
      return printyuju;
      console.log(printyuju);
      // document.getElementsByClassName()
    },
    // setInterval(passageToltal,3000);
  }
};
</script>
